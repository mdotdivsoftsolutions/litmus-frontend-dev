    ...
    <div className="animate-fade-in font-manrope bg-white min-h-screen">
      ...
      <section className="relative pt-20 pb-20 overflow-hidden bg-white">
        ...
        <div className="relative z-10 max-w-7xl mx-auto px-6 text-center">
            <div className="inline-flex items-center gap-2.5 px-4 py-2 rounded-2xl bg-white shadow-sm border border-slate-100 text-[#D32F2F] text-[10px] font-black uppercase tracking-[0.4em] mb-8">
               <Shield className="h-4 w-4" /> Curated Diagnostic Bundles
            </div>
            <h1 className="text-5xl lg:text-5xl font-semibold text-slate-800 tracking-tight leading-tight mb-8">
               Precision <span className="text-transparent bg-clip-text bg-gradient-to-r from-[#D32F2F] to-[#F06C00]">Clinical Packages</span> <br />
            ...
            <div className="flex flex-wrap justify-center gap-3">
              {categories.map((cat) => (
                <button
                  ...
                  className={cn(
                    "px-6 py-2 rounded-xl text-[10px] font-black uppercase tracking-[0.2em] transition-all duration-300 shadow-sm",
            ...
            <div key={pkg.id} className="group bg-white rounded-[3rem] p-8 border-2 border-slate-50 shadow-sm hover:shadow-[0_40px_80px_rgba(0,0,0,0.06)] hover:border-[#D32F2F]/20 transition-all duration-500 flex flex-col items-start lg:flex-row lg:items-stretch gap-10">
              <div className="flex-1 space-y-6">
                <div className="flex items-center justify-between">
                  <span className="bg-slate-100 text-slate-500 text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em]">{pkg.category}</span>
                  {pkg.tag && (
                    <span className="bg-red-50 text-[#D32F2F] text-[10px] font-black px-4 py-1.5 rounded-full uppercase tracking-[0.2em]">{pkg.tag}</span>
                  )}
                </div>
                <div>
                ...
                <div className="grid grid-cols-2 gap-4 py-6 border-y border-slate-100">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      <p className="text-xs text-slate-400 font-bold uppercase leading-none">Parameters</p>
                  ...
                  <div className="flex items-center gap-3 border-l border-slate-100 pl-4">
                    ...
                    <div>
                      <p className="text-xs text-slate-400 font-bold uppercase leading-none">Reports</p>
              ...
              <div className="w-full lg:w-48 bg-slate-50 rounded-[2rem] p-6 flex flex-col justify-between items-center text-center space-y-4">
                <div>
                  <span className="text-xs text-slate-400 line-through font-bold block">₹{pkg.mrp.toLocaleString()}</span>
                  <span className="text-3xl font-black text-slate-800 block mt-1 tracking-tighter">₹{pkg.price.toLocaleString()}</span>
                  <span className="inline-block mt-3 bg-emerald-100 text-emerald-600 text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-widest border border-emerald-200">
                    {discountPct(pkg.price, pkg.mrp)}% SAVING
                  </span>
                </div>
                <Button className="w-full h-11 rounded-xl bg-gradient-to-r from-[#D32F2F] to-[#feba50] text-white font-semibold text-sm shadow-[0_12px_24px_rgba(211,47,47,0.3)] hover:shadow-[0_16px_32px_rgba(211,47,47,0.4)] hover:-translate-y-0.5 active:scale-95 transition-all">
                  Book Panel
                </Button>
              </div>
            </div>
