...
import { Printer, ExternalLink, Copy, CheckCircle2, ShieldCheck, Download, AlertCircle, FileText } from "lucide-react";
...
export function InvoiceDialog({ bookingId, open, onOpenChange }: InvoiceDialogProps) {
  ...
  const handlePrint = () => {
    if (!invoice) return;
    // Create an invisible iframe to print the clean HTML representation
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[92vh] overflow-y-auto p-0 border border-slate-200 shadow-2xl rounded-2xl">
        <DialogHeader className="p-6 pb-4 border-b border-slate-100 bg-slate-50/70 sticky top-0 z-20 backdrop-blur-sm flex flex-row items-center justify-between">
          <div>
            ...
            <DialogDescription className="text-xs text-muted-foreground mt-0.5">
              GST-Compliant Commercial Invoice • SAC Code: 998346 (Technical Testing)
            </DialogDescription>
          </div>
          {invoice && (
            <div className="flex items-center gap-2 pr-6">
              ...
              <Button
                size="sm"
                onClick={handlePrint}
                className="h-8 text-xs gap-1.5 bg-primary text-white hover:bg-primary/90 shadow-xs font-semibold"
              >
            ...
            <div ref={printRef} className="space-y-6 font-sans">
              ...
              <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center pb-6 border-b-2 border-slate-900 gap-4">
                ...
                <div className="text-left sm:text-right">
                  ...
                  <div className="mt-2">
                    <Badge className={isPaid ? "bg-emerald-50 text-emerald-700 border-emerald-300 font-bold" : "bg-amber-50 text-amber-700 border-amber-300 font-bold"}>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-slate-50 p-4 rounded-xl border border-slate-200">
                ...
                <div>
                  <span className="text-[10px] font-bold uppercase text-slate-500 block">Booking Reference</span>
                  <span className="text-xs font-mono font-bold text-slate-800">{invoice.bookingId}</span>
                </div>
                <div>
                  <span className="text-[10px] font-bold uppercase text-slate-500 block">Transaction Reference</span>
              ...
              <div className="grid sm:grid-cols-2 gap-6 pt-2">
                <div className="bg-white p-4 rounded-xl border border-slate-200/80 shadow-2xs space-y-1.5">
                  <h4 className="text-[11px] font-extrabold uppercase tracking-wider text-emerald-800 pb-1 border-b border-slate-100 flex items-center gap-1.5">
